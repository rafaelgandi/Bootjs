...
if(left.id==='.'&&p.length>0&&left.first&&left.first.first&&are_similar(p[0],left.first.first)){if(left.second.string==='call'||(left.second.string==='apply'&&(p.length===1||(p[1].arity==='prefix'&&p[1].id==='[')))){warn('unexpected_a',left.second);}}}
...
var i,key,keys=Object.keys(property).sort(),length,output=['/*properties'],mem='    ',name,not_first=false;for(i=0;i<keys.length;i+=1){key=keys[i];if(property[key]>0){if(not_first){mem+=', ';}
